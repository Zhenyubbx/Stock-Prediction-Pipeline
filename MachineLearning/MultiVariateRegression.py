...
def use_linear_model(combined, stockName):
    ...
    # df_with_predicted.to_gbq("is3107-project-383009.Dataset.multiVariateLinearResults", project_id="is3107-project-383009", if_exists='replace')
    
    return df_with_predicted, training_time, predict_time_per_price, rmse, linear_model_Regr
...
def get_stock_data(transformed_df, stock):
...
def predict_with_linear_model(tdy_closing_price, stock):
    stockData, kaggleSentimentAnalysed = get_data()
    transformed_df = transform_df(stockData, kaggleSentimentAnalysed)
    stock_data = get_stock_data(transformed_df, stock)
    df_with_predicted, training_time, predict_time_per_price, rmse, linear_model_Regr = use_linear_model(stock_data, stock)
    real_time_analysed = query_table_from_bq("realTimeAnalysed110")
    tdy_compound = real_time_analysed['compound'].mean()
    predicted_price = linear_model_Regr.predict([[tdy_compound, float(tdy_closing_price)]]) #Predict today's closing price with ytd's closing price and ytd's compound
...
def analyse_with_linear_model(transformed_df):
    ...
    for StockName, group in grouped:
        ...
        # print(f"Successfully trained multi variate linear model for {StockName} group")
    ...
    # print(f"Successfully trained multi variate linear model for all groups")
    ...
    # analysed.to_gbq("is3107-project-383009.Dataset.multiVariateLinearResults", project_id="is3107-project-383009", if_exists='replace')
...
def analyse_with_random_forest(transformed_df):
    ...
    for StockName, group in grouped:
        ...
        # print(f"Successfully trained multi variate random forest model for {StockName} group")
        print(df_with_predicted.head(5))
        analysed = pd.concat([analysed, df_with_predicted])
        total_training_time = total_training_time + training_time
        total_predict_time_per_price = total_predict_time_per_price + predict_time_per_price
    ...
    average_training_time = total_training_time/num_of_stocks
    average_predict_time_per_price = total_predict_time_per_price/num_of_stocks
    # print(f"Successfully trained multi variate random forest model for all groups")
    # print(f"Average training time: {average_training_time}")
    # print(f"Average predict time per stock price: {average_predict_time_per_price}")
    # print(f"Average RSME: {average_rsme}")
    # print(analysed.head(5))
    # print(analysed.tail(5))
    analysed.to_gbq("is3107-project-383009.Dataset.multiVariateRandomForestResults", project_id="is3107-project-383009", if_exists='replace')

# stockData, kaggleSentimentAnalysed = get_data()
# grouped = transform_df(stockData, kaggleSentimentAnalysed)
# analyse_with_random_forest(grouped)
        
# test_predict_with_linear_model()


# use_linear_model(get_data())
